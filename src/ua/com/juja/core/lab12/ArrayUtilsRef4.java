...
public class ArrayUtilsRef4 {
    public static int[] lookFor(int[] array) {
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            if (index == 1) {
                result = сollateArrays(result, temporary);
                found = true;
            } else {
                // do nothing
            }
    ...
    private static int[] сollateArrays(int[] resultArray, int[] temporaryArray) {
        if (resultArray[1] - resultArray[0] < temporaryArray[1] - temporaryArray[0]) {
            return temporaryArray.clone();
        }
        return resultArray;
    }
}
